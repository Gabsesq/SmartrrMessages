...
/** TwiML: always plays the hosted recording from TWILIO_FAILED_PAYMENT_AUDIO_URL. */
function buildTwiml() {
...
/**
 * Outbound call options. Uses DetectMessageEnd so the same <Play> runs after a human
 * answers, or after voicemail greeting ends (beep / silence) — see Twilio AMD docs.
 * Set TWILIO_AMD_MODE=off to skip AMD (play starts immediately on connect; worse for voicemail).
 */
function buildOutboundCallOptions(phone, from) {
  const opts = {
    to: phone,
    from,
    twiml: buildTwiml(),
  };
  const amdMode = (process.env.TWILIO_AMD_MODE || "DetectMessageEnd").trim().toLowerCase();
...
/**
 * Place one outbound test call (same TwiML + AMD as production). No Supabase, no blocklist.
 * @param {string} toRaw - destination phone (any common US / E.164 shape)
 * @returns {Promise<{ callSid: string, to: string, from: string }>}
 */
async function placeTestOutboundCall(toRaw) {
  ...
  return { callSid: call.sid, to: phone, from };
}
